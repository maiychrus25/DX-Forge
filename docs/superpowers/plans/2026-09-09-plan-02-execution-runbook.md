...
- Whenever the `Dockerfile` or `.dockerignore` changes: `docker build -f apps/web/Dockerfile -t dx-forge-wizard:probe .`, run the image, and curl `/api/health`, `/login` and one database-writing endpoint. A Dockerfile that has never been built is not a deliverable (see deviation 11).
...
| 11 | 9 | Task 9 told the worker not to run `docker build`; the plan treats a reviewable Dockerfile as the deliverable | Docker turned out to be available, so the orchestrator built it — and it **failed**. `.dockerignore` excluded `**/dist` but not `**/*.tsbuildinfo`, so a stale build-state file was copied from the host while `dist/` was not; the root `prepare` ran `tsc -b`, saw "up to date", emitted nothing, and `next build` could then resolve neither workspace package. The same trap had already caught the orchestrator once on the host. | `.dockerignore` now excludes `**/*.tsbuildinfo`, and the image builds through `npm run build -w @dx-forge/web` so `apps/web`'s `prebuild` runs as a second line of defence. The image then built, ran, and passed a live smoke test: `/api/health` 200, `/pulse` 307 to `/login`, `/about` 200, admin login, an organisation written to SQLite, a round opened with three tokens, and the anonymous survey returning 18 questions with no cookie — which also proves the `better-sqlite3` native binding loads on the Debian base image. Gate tightened accordingly. |
| 12 | 10 | `/pulse/s/<unknown-token>` shows the generic error, but the E2E spec expects the "hết hạn" message | The route answers 404 for a token that was never issued and 410 only for expired/closed, while `SurveyForm` mapped 410 alone. Found by the Task 10 worker. | The **app** was corrected, not the spec: to a respondent, "never existed", "expired" and "round closed" are one event, and separating them also reveals whether a token ever existed. `SurveyForm` now treats 404 like 410 on both the GET and POST paths; the API keeps the status codes Task 4 specifies. `SurveyForm.tsx` joins Task 10's file list. |
| 13 | 10 | `expect(getByText("Radar HPDI").or(getByText(/Mức DTI/)))` | Both texts are on the closed-round page by design, so `.or()` always resolves to two elements and violates Playwright strict mode — the assertion could never pass. The very next line already asserts `Mức DTI \d/5`, so the `.or()` line was redundant as well. Found by the Task 10 worker, which refused to edit the test. | Orchestrator overruled the plan's test: the line now asserts the radar caption alone. |
| 14 | 10 | `playwright.config.ts` leaves `expect.timeout` at the 5s default and lets both projects run concurrently | Two failures in one. The first hit on a route Turbopack has not compiled takes 15–20s here, far past 5s. More seriously, the two projects share **one `next dev` server and therefore one SQLite file**, so running them concurrently is a data race, not merely a slow start — the worker reported the timing half, the orchestrator identified the shared-state half at the gate. | `fullyParallel: false`, `workers: 1`, `expect: { timeout: 15_000 }`. Suite then passed 4/4 on two consecutive runs, desktop and mobile. |
| 15 | 10 | — | `next dev` on Next 16 generates `apps/web/AGENTS.md` and `apps/web/CLAUDE.md` on every run. A second `AGENTS.md` inside `apps/web` would compete with this repository's own rules, which every worker is told to treat as the source of truth. | Both git-ignored. |
| | | | | |
